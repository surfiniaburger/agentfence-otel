...
function validateSnapshot(snapshot, asOf) {
  if (!snapshot?.is_complete || snapshot.parse_error != null) return false;
  if (!finiteNumber(snapshot.created_at)) return false;
  if (asOf !== null && !finiteNumber(asOf)) return false;
  if (snapshot.signatures?.length > 0 && !snapshot.nodes) return false;
  return true;
}

function isSignatureMalformed(sig, nodes) {
  if (!SUPPORTED_SINKS.has(sig.sink_type)) return true;
  if (!nodes?.[sig.source_id] || !nodes?.[sig.sink_id]) return true;
  if (sig.invalid_at != null && !finiteNumber(sig.invalid_at)) return true;
  return false;
}

function isSignatureActive(sig, evalTime) {
  return sig.invalid_at == null || sig.invalid_at > evalTime;
}

function getActiveSignatures(signatures, nodes, evalTime) {
  const active = [];
  for (const sig of signatures) {
    if (isSignatureMalformed(sig, nodes)) return null;
    if (isSignatureActive(sig, evalTime)) {
      active.push(sig);
    }
  }
  return active;
...
function hasValidProof(sig, nodes) {
  if (sig.source_type !== "UNTRUSTED_INPUT") return true;
  const sink = nodes[sig.sink_id] || {};
  return (
    sanitizerValidForSink(sig.sink_type, sig.sanitizer_type) &&
...
function allProofsValid(active, nodes) {
  return active.every((sig) => hasValidProof(sig, nodes));
}

export function evaluateSilverOneReachability(snapshot, asOf = null) {
  if (!validateSnapshot(snapshot, asOf)) return 1.0;

  const evalTime = asOf ?? snapshot.created_at;
  const signatures = snapshot.signatures || [];
  const active = getActiveSignatures(signatures, snapshot.nodes, evalTime);
  if (!active || active.length === 0) return 1.0;

  return allProofsValid(active, snapshot.nodes) ? 0.05 : 1.0;
}
